...
@ccclass
export default class Apple extends FoodBase {
    itemName: string = "apple"; 
    onLoad() {
        super.onLoad();
        cc.log(`[apple] onLoad → ${this.foodName || this.itemName}, rottenTime=${this.rottenTime}`);
        const def = getItemDefinition("apple");
        if (def) {
