...
@ccclass
export default class Greenapple extends FoodBase {
    itemName: string = "greenapple"; 
    onLoad() {
        super.onLoad();
        cc.log(`[greenapple] onLoad → ${this.foodName || this.itemName}, rottenTime=${this.rottenTime}`);
        const def = getItemDefinition("greenapple");
        if (def) {
