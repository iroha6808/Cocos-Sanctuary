...
@ccclass
export default class Grapes extends FoodBase {
    itemName: string = "grapes"; 
    onLoad() {
        super.onLoad();
        cc.log(`[grapes] onLoad → ${this.foodName || this.itemName}, rottenTime=${this.rottenTime}`);
        const def = getItemDefinition("grapes");
        if (def) {
