...
@ccclass
export default class Pineapple extends FoodBase {
    itemName: string = "pineapple"; 
    onLoad() {
        super.onLoad();
        cc.log(`[pineapple] onLoad → ${this.foodName || this.itemName}, rottenTime=${this.rottenTime}`);
        const def = getItemDefinition("pineapple");
        if (def) {
