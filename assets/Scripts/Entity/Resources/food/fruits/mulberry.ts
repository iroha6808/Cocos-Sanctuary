...
@ccclass
export default class Mulberry extends FoodBase {
    itemName: string = "mulberry"; 
    onLoad() {
        super.onLoad();
        cc.log(`[mulberry] onLoad → ${this.foodName || this.itemName}, rottenTime=${this.rottenTime}`);
        const def = getItemDefinition("mulberry");
        if (def) {
