...
@ccclass
export default class Peach extends FoodBase {
    itemName: string = "peach"; 
    onLoad() {
        super.onLoad();
        cc.log(`[peach] onLoad → ${this.foodName || this.itemName}, rottenTime=${this.rottenTime}`);
        const def = getItemDefinition("peach");
        if (def) {
