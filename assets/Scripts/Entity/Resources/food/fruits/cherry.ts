...
@ccclass
export default class Cherry extends FoodBase {
    itemName: string = "cherry"; 
    onLoad() {
        super.onLoad();
        cc.log(`[cherry] onLoad → ${this.foodName || this.itemName}, rottenTime=${this.rottenTime}`);
        const def = getItemDefinition("cherry");
        if (def) {
