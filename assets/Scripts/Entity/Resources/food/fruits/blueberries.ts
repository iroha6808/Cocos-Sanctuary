...
@ccclass
export default class Blueberries extends FoodBase {
    itemName: string = "blueberries"; 
    onLoad() {
        super.onLoad();
        cc.log(`[blueberries] onLoad → ${this.foodName || this.itemName}, rottenTime=${this.rottenTime}`);
        const def = getItemDefinition("blueberries");
        if (def) {
