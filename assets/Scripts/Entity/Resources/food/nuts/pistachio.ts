...
@ccclass
export default class Pistachio extends FoodBase {
    itemName: string = "pistachio"; 
    onLoad() {
        super.onLoad();
        cc.log(`[pistachio] onLoad → ${this.foodName || this.itemName}, rottenTime=${this.rottenTime}`);
        const def = getItemDefinition("pistachio");
        if (def) {
