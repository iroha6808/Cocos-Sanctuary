...
export default class AutoMapGenerator extends cc.Component {
    ...
    @property(cc.Float)
    generationStepInterval: number = 0.1;
    ...
    @property(cc.Float)
    cameraFrameDuration: number = 0.6;
    ...
    private spawnRock(root: cc.Node, placement: RockPlacement, index: number, prefix: string = "AutoRock_", source: "manual" | "box-generate" = "box-generate"): MapEditorPlacementState {
        ...
        root.addChild(node);
        return this.createPlacementState(node, "terrain", placement.spec.key, source);
    }

    private spawnResourceForPlacement(root: cc.Node, placement: RockPlacement, index: number, prefix: string = "AutoResource_", source: "manual" | "box-generate" = "box-generate"): MapEditorPlacementState {
    ...
    private createPlacementState(
        ...
        const world = node.parent
            ? node.parent.convertToWorldSpaceAR(node.position)
            : cc.v2(node.x, node.y);
        const local = targetRoot && cc.isValid(targetRoot)
            ? targetRoot.convertToNodeSpaceAR(world)
            : cc.v2(node.x, node.y);
        return {
            id: node.name,
            kind,
