...
- [ ] Menu 可加 `MenuEffectsRoot` 節點掛 `MenuRollingSpawner.ts`；`spawnRoot` 拖該節點或 Canvas，`rollingPrefabs` / `rollingSpriteFrames` 拖要從邊界噴出並滾過畫面的物件，調 `spawnInterval`、`maxAlive`、速度、scale、spin。
...
- [ ] GameOver 可加 `GameOverDecorRoot` 節點掛 `GameOverFloatingDecor.ts`；`floatingNodes` 可直接拖場景內要慢慢晃動的裝飾節點，不會淡出或消失；也可用 `decorPrefabs` / `decorSpriteFrames` 自動補裝飾，調 `initialCount`、`spawnInterval`、晃動幅度與 scale。
