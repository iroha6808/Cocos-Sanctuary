...
- `CameraRig.ts`
  ...
  - `frameWorldRect()` / `returnToTarget()` 可暫時切到 overview framing，用於自動地圖逐塊生成展示。
- `CameraFollow.ts`
...
- `Map/AutoMapGenerator.ts`
  ...
  - 預設不開場生成；Gameplay 按 `G` 後逐塊生成，每 `generationStepInterval` 秒 spawn 一塊。
  - 逐塊生成期間透過 `CameraRig.frameWorldRect()` 看完整生成範圍，完成後 `returnToTarget()` 回玩家。
...
- Rocksets
  ...
  - `autoGenerateOnStart` 預設關閉；GameManager 可拖 `autoMapGenerator`，Gameplay 按 `G` 逐塊生成，`generationStepInterval` 預設 0.07。
- UI Root
